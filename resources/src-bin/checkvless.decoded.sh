...
check_vless() {
    ...
    echo -e "${green}   .:::. VLESS USER LOGIN ACCOUNTS .:::.${neutral}"
    echo -e "${orange}─────────────────────────────────────────${neutral}"
    # read user database
    user_list=($(grep '^###' /etc/xray/vless/.vless.db | cut -d ' ' -f 2 | sort -u))
    >/tmp/rotation_vless
    ...
    done | sort -k1,1 -k3,3r >/tmp/rotation_vless

    # display results
    if [[ -s /tmp/rotation_vless ]]; then
        previous_user=""
        while read -r line; do
            read -r user ip_address access_time <<<"$line"
            if [[ -e /etc/xray/vless/${user} ]]; then
                current_time_seconds=$(date +%s.%N)
                access_time_seconds=$(date -d "${access_time}" +%s.%N)
                time_difference=$(echo "$current_time_seconds - $access_time_seconds" | bc)
                ...
                if (($(echo "$time_difference < 60" | bc -l))); then
                    if [[ "$user" != "$previous_user" ]]; then
                        ...
                        usage=$(</etc/xray/vless/usage/${user})
                        readable_usage=$(convert_size ${usage})
                        limit=$(</etc/xray/vless/${user})
                        readable_limit=$(convert_size ${limit})
                        connection_count=$(grep -cw "${user}" /tmp/rotation_vless)
