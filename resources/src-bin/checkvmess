...
check_vmess() {
    ...
    echo -e "${green}  .:::. VMESS USER LOGIN ACCOUNTS .:::.${neutral}"
    echo -e "${orange}─────────────────────────────────────────${neutral}"
    # read user database
    user_list=($(grep '^###' /etc/xray/vmess/.vmess.db | cut -d ' ' -f 2 | sort -u))
    >/tmp/rotation_vmess
    ...
    done | sort -k1,1 -k3,3r >/tmp/rotation_vmess

    # display results
    if [[ -s /tmp/rotation_vmess ]]; then
        previous_user=""
        while read -r line; do
            ...
            if [[ -e /etc/xray/vmess/${user} ]]; then
                ...
                if (($(echo "$time_difference < 60" | bc -l))); then
                    if [[ "$user" != "$previous_user" ]]; then
                        ...
                        usage=$(</etc/xray/vmess/usage/${user})
                        readable_usage=$(convert_size ${usage})
                        limit=$(</etc/xray/vmess/${user})
                        readable_limit=$(convert_size ${limit})
                        connection_count=$(grep -cw "${user}" /tmp/rotation_vmess)
